...
func NewVariable(ident string, raw string, env Env, restrict *Restrictions) *VariableNode {
	return &VariableNode{NodeVariable, ident, raw, env, restrict}
}

func (t *VariableNode) GetValue() string {
...
func (t *VariableNode) validateNoUnset() error {
	if t.Restrict.ErrorOnUnset && !t.isSet() {
...
func (t *VariableNode) validateNoEmpty(value string) error {
	if t.Restrict.ErrorOnEmpty && value == "" && t.isSet() {
...
func (t *SubstitutionNode) String() (string, error) {
	if t.ExpType >= itemPlus && t.Default != nil {
		switch t.ExpType {
		case itemColonDash, itemColonEquals: // If var not set or is empty, evaluate expression as $DEFAULT
			if !t.Variable.isSet() || t.Variable.GetValue() == "" {
				return t.Default.String()
			} else if s, _ := t.Variable.String(); s != "" {
				return s, nil
			}
		case itemPlus, itemColonPlus: // If var set, evaluate expression as $OTHER, otherwise as empty string
			if t.Variable.isSet() {
				return t.Default.String()
			}
			return "", nil
		default:
			if !t.Variable.isSet() { // If var not set, evaluate expression as $DEFAULT
				return t.Default.String()
			}
		}
	}
