// Package parse Most of the code in this package taken from golang/text/template/parse
package parse
...
// Mode A mode value is a set of flags (or 0). They control parser behavior.
type Mode int
...
// Restrictions The restrictions option controls the parsring restriction.
...
Loop:
	for {
		...
		case itemVariable:
			varNode := NewVariable(strings.TrimPrefix(t.val, "$"), t.val, p.Env, p.Restrict)
...
func (p *Parser) action() (Node, error) {
	...
	t := p.next()
	varNode := NewVariable(t.val, "", p.Env, p.Restrict)
	_start := t.pos
Loop:
	for {
		switch t = p.next(); t.typ {
		...
		case itemVariable:
			defaultNode = NewVariable(strings.TrimPrefix(t.val, "$"), t.val, p.Env, p.Restrict)
		case itemText:
			n := NewText(t.val)
		Text:
			for {
	...
	_end := t.pos
	varNode.Raw = "${" + p.lex.input[_start:_end] + "}"
